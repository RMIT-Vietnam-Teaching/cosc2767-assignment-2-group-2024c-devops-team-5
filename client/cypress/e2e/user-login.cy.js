...
describe('Login Functionality', () => {
  ...
  describe('Login Form', () => {
    it('should login successfully with valid credentials', () => {
      // Fill out login form with more specific selectors
      cy.get('input[name="email"]')
        .first()
        .should('exist')
        .type('newuser@example.com');
      ...
      // Submit form with more specific selector
      cy.get('button[type="submit"]')
        .first()
        .should('contain', 'Sign In')
        .click();
    ...
    it('should display validation errors for empty fields', () => {
      ...
      // Check for error messages with more specific selectors
      cy.get('input[name="email"]')
        .first()
        .parents('.input-box')
        .find('.invalid-message')
    ...
    it('should validate email format', () => {
      // Type invalid email with more specific selectors
    ...
    it('should validate minimum password length', () => {
      ...
      cy.get('input[name="password"]').first().type('12345');
      cy.get('button[type="submit"]').first().click();
    ...
    it('should show loading state during submission', () => {
      ...
      // Check loading indicator
      cy.get('.loading-indicator').first().should('exist');
    });
  ...
  describe('Authentication State', () => {
    ...
    it('should clear form after successful login', () => {
      ...
      cy.get('input[name="password"]').first().should('have.value', '');
    });
    ...
    it('should handle failed login attempt', () => {
      ...
      cy.get('input[name="email"]').first().type('wrong@example.com');
